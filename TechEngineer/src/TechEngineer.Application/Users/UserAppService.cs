...
using TechEngineer.Authorization.Accounts;
...
namespace TechEngineer.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        public UserAppService(
            ...
            LogInManager logInManager, IObjectMapper objectMapper)
            : base(repository)
        {
        ...
        public override async Task<PagedResultDto<UserDto>> GetAllAsync(PagedUserResultRequestDto input)
        {
            ...
            if (roles.Contains(StaticRoleNames.Tenants.SuperAdmin))
            {
                ...
                {
                    var t = Repository.GetAll().Where(x => x.OrganizationId == input.OrganizationId).PageBy(input).ToList();
            ...
            {
                var t = Repository.GetAll().Where(x => x.OrganizationId == input.OrganizationId && x.IsActive == true).PageBy(input).ToList();
                return new PagedResultDto<UserDto>
                {
                    TotalCount = t.Count(),
                    Items = _objectMapper.Map<List<UserDto>>(t)
        ...
        /// </summary>
        /// <param name="input">Create user dto.</param>
        /// <returns>Return user model.</returns>
        public async Task<UserDto> CreateStoreAdmin(CreateUserDto input)
        {
            CheckCreatePermission();

            var Roles = new string[] { StaticRoleNames.Tenants.StoreAdmin.ToUpper() };
        ...
        /// <param name="input">Create user dto.</param>
        /// <returns>Return user model.</returns>
        public async Task<UserDto> CreateStoreUser(CreateUserDto input)
        {
            var Roles = new string[] { StaticRoleNames.Tenants.StoreUser.ToUpper() };
            input.RoleNames = Roles;
